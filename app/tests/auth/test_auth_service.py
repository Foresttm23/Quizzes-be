...
def test_verify_token_and_get_payload_local_success(auth_service, mock_auth_repository):
    mock_payload = {"id": "123", "email": "test@local.com"}
    mock_auth_repository.verify_local_token_and_get_payload.return_value = mock_payload

    result = auth_service.verify_token_and_get_payload("valid_local_token")

    assert result == mock_payload
    mock_auth_repository.verify_auth0_token_and_get_payload.assert_not_called()


def test_verify_token_and_get_payload_auth0_fallback_success(auth_service, mock_auth_repository):
    mock_payload = {"email": "test@auth0.com"}
    mock_auth_repository.verify_local_token_and_get_payload.side_effect = InvalidJWTException
    mock_auth_repository.verify_auth0_token_and_get_payload.return_value = mock_payload

    result = auth_service.verify_token_and_get_payload("valid_auth0_token")

    assert result == mock_payload
    mock_auth_repository.verify_local_token_and_get_payload.assert_called_once()
    mock_auth_repository.verify_auth0_token_and_get_payload.assert_called_once()


def test_verify_token_and_get_payload_all_fail(auth_service, mock_auth_repository):
    mock_auth_repository.verify_local_token_and_get_payload.side_effect = InvalidJWTException
    mock_auth_repository.verify_auth0_token_and_get_payload.side_effect = InvalidJWTException
...
@pytest.mark.asyncio
async def test_handle_jwt_sign_in_user_exists(auth_service, mock_user_service, mock_user):
    ...
    mock_user_service.create_user_from_jwt.assert_not_called()


@pytest.mark.asyncio
async def test_handle_jwt_sign_in_user_not_found(auth_service, mock_user_service, mock_user):
    ...
    mock_user_service.create_user_from_jwt.assert_called_once_with(user_info=jwt_payload)


def test_create_access_token_calls_repo_correctly(mocker, auth_service, mock_user, mock_auth_repository):
