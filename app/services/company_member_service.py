...
class CompanyMemberService(BaseService[CompanyMemberRepository]):
    ...
    async def fetch_company_members_data_paginated(self, page: int, page_size: int, company_id: UUID):
    ...
    async def remove_user_from_company(self, company_id: UUID, acting_user_id: UUID, user_id: UUID) -> None:
        ...
        user_role = await self.repo.get_user_role(company_id=company_id, user_id=user_id)
        await self.assert_user_has_role(company_id=company_id, user_id=acting_user_id, required_role=user_role)

        await self.repo.remove_user_from_company(company_id=company_id, user_id=user_id)
    ...
    async def leave_company(self, company_id: UUID, user_id: UUID) -> None:
        ...
        if user_role == CompanyRole.OWNER:
            # TODO can be later updated to give owner role to some user/admin if there are no other owners, since app supports many owners.
            pass
    ...
    async def assert_user_in_company(self, company_id: UUID, user_id: UUID) -> None:
        company_member = await self.repo.get_user_from_company(company_id=company_id, user_id=user_id)
        if not company_member:
            raise UserIsNotACompanyMemberException()

    async def assert_user_has_role(self, company_id: UUID, user_id: UUID, required_role: CompanyRole) -> None:
        role = await self.repo.get_user_role(company_id=company_id, user_id=user_id)
        if role is None:
            raise UserIsNotACompanyMemberException()
