...
async def get_members_paginated(member_service: CompanyMemberServiceDep, company_id: UUID,
    ...
    company_members = await member_service.get_members_paginated(page=page, page_size=page_size,
...
@router.patch("/{company_id}/{target_user_id}", response_model=CompanyMemberDetailsResponse,
              status_code=status.HTTP_200_OK, )
async def update_member_role(member_service: CompanyMemberServiceDep, acting_user: GetUserJWTDep,
