...
from app.core.dependencies import GetUserJWTDep, CompanyQuizServiceDep
from core.config import settings
from schemas.base_schemas import PaginationResponse
from schemas.company.quiz.question_schema import QuestionUserResponseSchema, QuestionUpdateRequestSchema, \
    QuestionAdminResponseSchema, QuestionCreateRequestSchema
from schemas.company.quiz.quiz_schema import QuizCreateRequestSchema, QuizDetailsResponseSchema, QuizUpdateRequestSchema
...
async def get_quiz(quiz_service: CompanyQuizServiceDep, company_id: UUID, quiz_id: UUID):
    quiz = await quiz_service.get_quiz(company_id=company_id, quiz_id=quiz_id)
    return quiz


@router.get("/{company_id}/quizzes", response_model=PaginationResponse[QuizDetailsResponseSchema],
            status_code=status.HTTP_200_OK)
async def get_company_quizzes(quiz_service: CompanyQuizServiceDep, user: GetUserJWTDep, company_id: UUID,
...
@router.patch("/{company_id}/{quiz_id}/quiz", response_model=QuizDetailsResponseSchema, status_code=status.HTTP_200_OK)
