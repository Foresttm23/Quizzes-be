...
from app.schemas.company.inv_req_schema import (InvitationDetailsResponse, AcceptInvitationResponse,
...
@router.get("/user/pending", response_model=PaginationResponse[InvitationDetailsResponse],
            status_code=status.HTTP_200_OK, )
async def get_user_pending_invitations(company_invitation_service: CompanyInvitationServiceDep, user: GetUserJWTDep,
                                       page: int = Query(default=1, ge=1),
                                       page_size: int = Query(default=10, ge=1, le=settings.APP.MAX_PAGE_SIZE), ):
