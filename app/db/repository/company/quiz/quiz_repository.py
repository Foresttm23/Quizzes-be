...
class CompanyQuizRepository(BaseRepository[QuizModel]):
    ...
    async def get_last_version_number(
            self, company_id: UUID, root_id: UUID
    ) -> int | None:
        query = select(func.max(QuizModel.version)).where(
            QuizModel.company_id == company_id,
            or_(QuizModel.root_quiz_id == root_id, QuizModel.id == root_id),
        )
        current_max = await self.db.scalar(query)
        return current_max

    async def get_publish_status(self, company_id: UUID, quiz_id: UUID) -> bool | None:
        query = select(QuizModel.is_published).where(
    ...
    async def get_question_or_none(
            self, company_id, quiz_id: UUID, question_id: UUID
    ) -> QuestionModel | None:
        query = (
            select(QuestionModel)
            .join(QuizModel, QuestionModel.quiz_id == QuizModel.id)
            .where(
    ...
    async def get_questions_and_options(
        ...
        query = (
            select(QuestionModel)
            .join(QuizModel)
            .where(QuestionModel.quiz_id == quiz_id, QuizModel.company_id == company_id)
