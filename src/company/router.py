...
from .schemas import CompanyCreateRequestSchema, CompanyUpdateInfoRequestSchema, CompanyDetailsResponseSchema, \
    RequestDetailsResponse, AcceptRequestResponse, InvitationDetailsResponse, AcceptInvitationResponse, \
    CreateInvitationRequest, CompanyMemberDetailsResponse, UpdateMemberRoleSchema
...
@companies_router.post("/", response_model=CompanyDetailsResponseSchema, status_code=status.HTTP_201_CREATED)
...
@companies_router.get("/{company_id}", response_model=CompanyDetailsResponseSchema, status_code=status.HTTP_200_OK)
...
@companies_router.patch("/{company_id}", response_model=CompanyDetailsResponseSchema, status_code=status.HTTP_200_OK)
...
@companies_router.post("/{company_id}/invitations", response_model=InvitationDetailsResponse,
...
@invitations_router.patch("/{invitation_id}/accept", response_model=AcceptInvitationResponse,
                          status_code=status.HTTP_200_OK, )
...
@invitations_router.patch("/{invitation_id}/decline", response_model=InvitationDetailsResponse,
                          status_code=status.HTTP_200_OK, )
...
@invitations_router.patch("/{invitation_id}/cancel", response_model=InvitationDetailsResponse,
                          status_code=status.HTTP_200_OK, )
...
@companies_router.post("/{company_id}/join-requests", response_model=RequestDetailsResponse,
                       status_code=status.HTTP_200_OK, )
async def create_join_request(company_join_request_service: CompanyJoinRequestServiceDep, user: GetUserJWTDep,
                              company_id: UUID, ):
    request = await company_join_request_service.create_join_request(company_id=company_id, requesting_user_id=user.id)
    return request


@requests_router.patch("/{request_id}/accept", response_model=AcceptRequestResponse, status_code=status.HTTP_200_OK, )
...
@requests_router.patch("/{request_id}/decline", response_model=RequestDetailsResponse, status_code=status.HTTP_200_OK, )
...
@requests_router.patch("/{request_id}/cancel", response_model=RequestDetailsResponse, status_code=status.HTTP_200_OK, )
...
async def remove_member(member_service: CompanyMemberServiceDep, acting_user: GetUserJWTDep, company_id: UUID,
                        target_user_id: UUID, ):
    if acting_user.id == target_user_id:
        await member_service.leave_company(company_id=company_id, user_id=acting_user.id)
    else:
        await member_service.remove_member(company_id=company_id, acting_user_id=acting_user.id,
                                           target_user_id=target_user_id, )


@companies_router.patch("/{company_id}/members/{target_user_id}", response_model=CompanyMemberDetailsResponse,
                        status_code=status.HTTP_200_OK, )
async def update_member_role(member_service: CompanyMemberServiceDep, acting_user: GetUserJWTDep, company_id: UUID,
                             target_user_id: UUID, new_data: UpdateMemberRoleSchema, ):
    member = await member_service.update_role(company_id=company_id, target_user_id=target_user_id,
