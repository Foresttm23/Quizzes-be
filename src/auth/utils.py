...
def encode_access_token(data: dict, expires_delta: timedelta, local_settings: LocalJWTSettings) -> str:
    """Creates a signed JWT access token."""
    expire = datetime.now(timezone.utc) + expires_delta
    data.update({"exp": expire, "type": JWTTypeEnum.ACCESS, "auth_provider": AuthProviderEnum.LOCAL, })

    encoded_jwt = _handle_local_token_encode(data=data, secret=local_settings.LOCAL_JWT_SECRET,
                                             algorithm=local_settings.LOCAL_JWT_ALGORITHM)
    return encoded_jwt


def encode_refresh_token(data: dict, expires_delta: timedelta, local_settings: LocalJWTSettings) -> str:
    expire = datetime.now(timezone.utc) + expires_delta
    data.update({"exp": expire, "type": JWTTypeEnum.REFRESH})
    encoded_jwt = _handle_local_token_encode(data=data, secret=local_settings.LOCAL_REFRESH_TOKEN_SECRET,
                                             algorithm=local_settings.LOCAL_JWT_ALGORITHM)

    return encoded_jwt


def verify_local_token_and_get_payload(token: str, local_settings: LocalJWTSettings) -> dict:
    payload = _handle_local_token_decode(token=token, secret=local_settings.LOCAL_JWT_SECRET,
                                         algorithm=local_settings.LOCAL_JWT_ALGORITHM)
    return payload


def verify_refresh_token_and_get_payload(token: str, local_settings: LocalJWTSettings) -> dict:
    payload = _handle_local_token_decode(token=token, secret=local_settings.LOCAL_REFRESH_TOKEN_SECRET,
                                         algorithm=local_settings.LOCAL_JWT_ALGORITHM)
    return payload


async def verify_auth0_token_and_get_payload(token: str, auth0_settings: Auth0JWTSettings,
                                             http_client: httpx.AsyncClient) -> dict:
    payload = await _handle_auth0_token_decode(token=token, jwks_endpoint=auth0_settings.AUTH0_JWKS_ENDPOINT,
                                               audience=auth0_settings.AUTH0_JWT_AUDIENCE,
                                               algorithm=auth0_settings.AUTH0_JWT_ALGORITHM, http_client=http_client)
    return payload


def generate_user_id_from_auth0(auth0_sub: str, uuid_secret: UUID) -> UUID:
...
async def _handle_auth0_token_decode(token: str, jwks_endpoint: str, audience: str, algorithm: str,
    ...
    try:
        ...
        public_key = await _find_public_key(kid=kid, jwks_endpoint=jwks_endpoint, http_client=http_client)
...
async def _find_public_key(kid: str, jwks_endpoint: str, http_client: httpx.AsyncClient) -> dict | None:
